...
ULWindow ulCreateWindow(ULMonitor monitor, ULString name, unsigned int width,
                        unsigned int height, bool fullscreen, unsigned int window_flags)
{
  return new C_Window(Window::Create(reinterpret_cast<Monitor *>(monitor), ToString(name).utf8().data(), width, height, fullscreen, window_flags));
}
